...
txamp=twix_obj.hdr.Meas.TransmitterReferenceAmplitude;
...
elseif isMinn && ~(Bo == 6.98) % CWJ: Slight issue in Twix data for 7T CMRR
...
%CWJ Added voxel postition info for tissue correction
out.p.VoI_InPlaneRot = CHECK(twix_obj.hdr.Config.VoI_InPlaneRotAngle);
out.p.NormCor = CHECK(twix_obj.hdr.Config.VoI_Normal_Cor);
out.p.NormSag = CHECK(twix_obj.hdr.Config.VoI_Normal_Sag);
out.p.NormTra = CHECK(twix_obj.hdr.Config.VoI_Normal_Tra);
out.p.voxdim = [CHECK(twix_obj.hdr.Config.VoI_PeFOV), CHECK(twix_obj.hdr.Config.VoI_RoFOV), CHECK(twix_obj.hdr.Config.VoI_SliceThickness)];
out.p.voxoff = [CHECK(twix_obj.hdr.Config.VoI_Position_Sag), CHECK(twix_obj.hdr.Config.VoI_Position_Cor) , CHECK(twix_obj.hdr.Config.VoI_Position_Tra)];
if isfield(twix_obj.hdr.Config , 'VoI_GlobalTablePosSag')
    out.p.TablePosition = [CHECK(twix_obj.hdr.Config.GlobalTablePosSag) , CHECK(twix_obj.hdr.Config.GlobalTablePosCor) , CHECK(twix_obj.hdr.Config.GlobalTablePosTra)];
else
    out.p.TablePosition = [0,0,0];
end

try %CWJ Very bad... Fix this later
    out.p.alFree = twix_obj.hdr.MeasYaps.sWipMemBlock.alFree; %CWJ-Grab sequence/special parameters
end
