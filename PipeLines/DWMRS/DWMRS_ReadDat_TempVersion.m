function[Out_Structrure , QA] = DWMRS_ReadDat_TempVersion(IN , OutDir, WaterScan)
...
if ~(length(unique(abs(G))) == length(G)) %if repeated nominal G amplitude
    Remove=[];
    if true%length(unique(G)) == length(unique(abs(G))) % Inhomogeneous averaging
       [~,Ind]=unique(abs(G));
       DegenVals = abs(G(setdiff(1:length(G),Ind'))); %Find repeated values
       [~,DLoc] = ismember(abs(All_G), DegenVals); %Locations of these values

       for K=1:length(DegenVals) %Loop over repeated elements and directions and combine spectra
           DegenSet = reshape(find(DLoc==K),length(find(DLoc==K))/Ndir,Ndir)';
           for J=1:Ndir
                Spec_cc{DegenSet(J,1)} = Combine_Inhomo(Spec_cc(DegenSet(J,:)));
                if Water_reference
                   Specw_cc{DegenSet(J,1)} = Combine_Inhomo(Specw_cc(DegenSet(J,:)));
                end
            end
    ...
    % Redefine parameters accordingly
    Ng=length(G);  RepSize = Ndir*Ng+1; All_G = [0 ,repmat(G,[1,Ndir])];
    All_Dir = [0,reshape(repmat(cell2mat(RawData.p.alFree(15:15+Ndir-1)), [Ng,1]),[Ng*Ndir,1])'];  
...
for J=1:RepSize
    Spec_cc2{J} = Spec_cc{J};
    Specw_cc2{J} = Spec_cc{J};
    iter=1;nbadAverages=1;nBadAvgTotal=0;
    while nbadAverages>0
        [Spec_cc2{J},metric,badAverages]=op_rmbadaverages(Spec_cc2{J},BadAv_nsd,'t',1);
